...
@st.cache_data
def load_reco_data():
    return pd.read_csv("csv/df_reco_film.csv", index_col=0)
...
def reco():
    df = st.session_state["df_final"]
    df_reco = st.session_state["df_reco_film"]
    session_states()
...
# --- Recommandations depuis df_reco_film ---
    st.markdown("---")
    st.subheader("🎯 Films recommandés")

    # Récupérer les recommandations
    try:
        rec_titles = eval(df_reco.loc[film.name][0])  # Liste de titres recommandés
    except:
        st.info("Aucune recommandation disponible pour ce film.")
        return

    # Limiter aux films disponibles dans df
    reco_films = df[df.index.isin(rec_titles)]
